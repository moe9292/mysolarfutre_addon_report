...
with st.form("upgrade_form"):
    ...
    c1, c2, c3 = st.columns(3)
    upgrade_price = c1.number_input("Preis Standard (EUR, inkl. Anmeldung, Basis 8 Module)", 0, 20000, 4448, step=50)
    entry_price = c2.number_input("Preis Einstieg (EUR, inkl. Anmeldung, Basis 8 Module)", 0, 20000, 3848, step=50)
    electricity_price = c3.number_input("Strompreis (EUR/kWh)", 0.10, 1.00, 0.34, step=0.01)
    st.caption("Bei Ziel-Modulzahl 6 zieht der Bericht automatisch 1.000 € von beiden Preisen ab.")
    c1, c2, c3 = st.columns(3)
